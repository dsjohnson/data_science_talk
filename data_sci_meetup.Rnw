...
$V$ has some mathy constraints such as nonnegative-definitness, that is $a'V a \ge 0$ for any $a$.
...
\begin{itemize}
\medsep
\item Gaussian processes are usually too burdonsome to work with for real problems
...
There are many different types of penalized spline smoothers: cubic, thin-plate, 
...
\sectitle{Gaussian Markov random fields} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{Modeling functions discretely}
What if $x$ isn't continuous? Then $f(x)$ only makes sense for, say, $x_1,\dots,x_k$
...
\tikz\node[opacity=0.4, inner sep=0]{\includegraphics[width=\paperwidth, height=\paperheight, scale=1]{nfs_pup_questions.png}};}
\begin{frame}
\textcolor{slate}{\Huge\bf Questions?}
\end{frame}
}
